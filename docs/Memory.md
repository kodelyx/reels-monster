...
## 📝 Log
- **2026-07-16** — Audio normalization, phonetic merges, dynamic SFX and cleanup. (a) Added `-14 LUFS` loudness normalization to `06_process/run.py` to ensure volume consistency across scene avatar clips. (b) Integrated phonetic merge rules (ए+आई→AI, etc.) directly into `06_process/run.py` so captions display correct tech terms. (c) Moved `sound_manager.py` to `stages/07_popups/` and updated it to use local config paths. (d) Upgraded `07_popups/run.py` to dynamically scan `sfx/` folder and build a mood-categorized catalog, letting AI pick context-aware sounds for each popup instead of hardcoded keys, and dynamically syncing new keys to Remotion's `PopupAsset.tsx`. (e) Fully deleted the obsolete `_archive/` directory. (f) Updated all project documentation (`PRD.md`, `Architecture.md`, `Rules.md`, `Phases.md`, `Memory.md`) to represent the clean pipeline structure.
